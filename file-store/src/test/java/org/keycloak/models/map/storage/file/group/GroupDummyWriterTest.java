...
//        childGroup.setGrantedRoles(Set.of("role4"));
        ...
        List<Event> events = new LinkedList<>();
        addStartEvents(events);
        addPairOfScalars(events, "schemaVersion", "1");

        for (MapGroupEntityFields field : MapGroupEntityFields.values()) {
            System.out.println("  adding field " + field.getName());
            if (field.equals(MapGroupEntityFields.ID) || field.equals(MapGroupEntityFields.REALM_ID)) {
            ...
            Object fieldVal = field.get(parentGroup);
            if (fieldVal != null) { // field has some non-null value
                // let's start with those used in MapGroupEntityFields only
                if (field.getFieldClass().equals(String.class)) { // NAME, PARENT_ID this should be pair of scalars
                    addPairOfScalars(events, field.getNameCamelCase(), fieldVal.toString());
                } else if (field.getFieldClass().equals(Set.class)) { // GRANTED_ROLES
                    String keyName = field.getNameCamelCase();
                    Set<String> values = (Set) field.get(parentGroup); // for now we know it's Set<String> - TODO make it more general

                    if (values.size() == 1) {
                        addPairOfScalars(events, keyName, values.iterator().next());
                    } else {
                        addSequence(events, keyName, values);
                    }
                } else if (field.getFieldClass().equals(Map.class)) { // ATTRIBUTES
                    addAttributes(events, parentGroup);
                }
            }
        }
    ...
    private void addAttributes(List<Event> events, MapGroupEntity group) {
        String attributesKey = MapGroupEntityFields.ATTRIBUTES.getNameCamelCase();
        Map<String, List<String>> attributes = (Map) MapGroupEntityFields.ATTRIBUTES.get(group); // we know attributes are Map<String, List<String>>
