...
import org.keycloak.models.map.storage.file.YamlContext.AbstractYamlContext;
 ...
 * @author hmlnarik
 */
public class MapEntityYamlContext<T> extends AbstractYamlContext<T> {
    ...
    public MapEntityYamlContext(String name, Class<T> clazz) {
        super(name);
        this.result = (T) AutogeneratedClasses.CONSTRUCTORS_DC.get(clazz).apply(DeepCloner.DUMB_CLONER);    // TODO: Introduce support in DeepCloner
    ...
    private static <T> Supplier<? extends YamlContext<?>> getDefaultContextCreator(EntityField<? super T> ef) {
        final Class<?> collectionElementClass = ef.getCollectionElementClass(); // collection
        if (collectionElementClass != Void.class) {
            if (AutogeneratedClasses.ENTITY_FIELDS.containsKey(collectionElementClass)) {
                return () -> new MapEntitySequenceYamlContext<>(ef.getNameCamelCase(), collectionElementClass);
            }
        } // <end>

        final Class<?> mapValueClass = ef.getMapValueClass();
        if (mapValueClass != Void.class) { // map
            if (AutogeneratedClasses.ENTITY_FIELDS.containsKey(mapValueClass)) { // entity
                return () -> new MapEntityMappingYamlContext<>(ef.getNameCamelCase(), mapValueClass);
            } else if (ATTRIBUTES_NAME.equals(ef.getName())) { //attributes
    ...
    public YamlContext<?> getContext(String nameOfSubcontext) {
        ...
        if (ef != null) {
            if (ef.getCollectionElementClass() != Void.class) {
                return new DefaultListContext(nameOfSubcontext);
            } else if (ef.getMapValueClass() == List.class) {
                return new DefaultMapContext(nameOfSubcontext);
            }
            return null;
        }
    ...
    public static class MapEntitySequenceYamlContext<T> extends DefaultListContext {
        ...
        public MapEntitySequenceYamlContext(String name, Class<T> collectionElementClass) {
            super(name);
        ...
        @Override
        public YamlContext<?> getContext(String nameOfSubcontext) {
            return new MapEntityYamlContext<>(nameOfSubcontext, collectionElementClass);
        }
    ...
    private static class MapEntityMappingYamlContext<T> extends DefaultMapContext {

        private final Class<T> mapValueClass;

        public MapEntityMappingYamlContext(String name, Class<T> mapValueClass) {
            super(name);
        ...
        @Override
        public YamlContext<?> getContext(String nameOfSubcontext) {
            return new MapEntityYamlContext<>(nameOfSubcontext, mapValueClass);
        }
    }
